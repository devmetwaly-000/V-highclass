...
import { Repeat, Filter } from 'lucide-react';
...
const getTopRentedProducts = (products: Product[], orders: Order[], fromDate?: Date, toDate?: Date): ProductCount[] => {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    return isRental && dateMatch;
  });

  filteredOrders.forEach(order => {
    order.items.forEach(item => {
      rentalCounts.set(item.productId, (rentalCounts.get(item.productId) || 0) + item.quantity);
    });
  });
...
function TopRentedProductsPageContent() {
    ...
    const topProducts = useMemo(() => {
        if (isLoading) return [];
        return getTopRentedProducts(products, orders, fromDate, toDate);
    }, [products, orders, isLoading, fromDate, toDate]);

  return (
    <div className="flex flex-col gap-8">
      <PageHeader title="تقرير الأصناف الأكثر إيجارًا" showBackButton />
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle>الأصناف الأكثر إيجارًا</CardTitle>
          </div>
          <CardDescription>
            قائمة بالمنتجات مرتبة تنازليًا حسب عدد مرات الإيجار في الفترة المحددة.
          </CardDescription>
        </CardHeader>
        <CardContent>
           <Table>
               ...
               {!isLoading && topProducts.length === 0 && (
                <TableRow>
                    <TableCell colSpan={3} className="h-24 text-center text-muted-foreground">
                        لم يتم تسجيل أي إيجارات في هذه الفترة.
                    </TableCell>
                </TableRow>
               )}
            </TableBody>
...
export default function TopRentedProductsPage() {
    return (
        <AppLayout>
            <TopRentedProductsPageContent />
        </AppLayout>
    )
}
